...
case class PgUserBookRepository(ds: DataSource) extends UserBookRepository:
      ...
      for {
        ...
        result <-
          run {
            quote {
              query[UsersBooks]
                .insertValue(lift(UsersBooks(uuid, userId, bookId, inLib, progress, rating)))
                .returning(ub => ub.id)
            }
          }
      } yield result
  ...
  override def addToLibrary(
    ...
    transaction {
      findUserBook(userId, bookId) flatMap {
        case Some(id) => setInLibraryStatus(id, true)
  ...
  override def deleteFromLibrary(
    ...
    transaction {
      findUserBook(userId, bookId) flatMap {
        case Some(id) => setInLibraryStatus(id, false)
        case None     => ZIO.unit
      }
    }.provide(dsLayer)
  ...
  private def setInLibraryStatus(
    ...
    run {
      quote {
        query[UsersBooks]
          .filter(ub => ub.id == lift(userBookId))
  ...
  override def setProgress(
    ...
    transaction {
      findUserBook(userId, bookId) flatMap {
        case Some(id) => setProgressStatus(id, progress)
  ...
  override def setRating(
    ...
    transaction {
      findUserBook(userId, bookId) flatMap {
        case Some(id) => setRatingStatus(id, rating)
